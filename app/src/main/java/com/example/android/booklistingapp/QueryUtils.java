...
public final class QueryUtils {
     ...
     */
    private static List<BookListing> extractFeatureFromJson(String itemsJSON) {
        ...
        try {
            ...
            final JSONArray volumeInfoArray = new JSONObject(itemsJSON).getJSONObject("items").getJSONArray("volumeInfo");

            // For each book in the booklistingArray, create an {@link BookListing} object
            for (int i = 0; i < volumeInfoArray.length(); i++) {

                // Get a single book at position i within the list of books
                JSONObject currentBookListing = volumeInfoArray.getJSONObject(i);

                // For a given book, extract the JSONObject associated with the
                // key called "items", which represents a list of all "items"
                // for that book.
                JSONObject items = currentBookListing.getJSONObject("items");
                ...
                if(items.has("title")) {
                    ...
                    // Create a new {@link BookListing} object with the title, authors,
                    // from the JSON response.
                    BookListing booklisting = new BookListing (title, authors);
